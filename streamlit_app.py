...
class Streamlit():
    ...
    def create_app_interface(self):
        ...
        # =====================================
            ...
            st.markdown("""
            In dieser Anwendung vergleichen wir **dynamische** und **statische Energiepreise** unter verschiedenen Bedingungen:
            
            - **Dynamische Preise**: ändern sich stündlich basierend auf dem Börsenstrompreis.
            - **Statische Preise**: bleiben konstant über den Abrechnungszeitraum.
            - **PV-Anlage & Batterie**: ermöglichen Eigenverbrauch und Speicherung.
            - **EEG-Vergütung**: berücksichtigt Einspeisung ins Netz.

            Die Berechnung berücksichtigt typische Lastprofile, PV-Erzeugung nach Ausrichtung und Standort sowie Batteriespeicherverhalten.
            """)
        ...
        # =====================================
            ...
            if not st.session_state.results:
                ...
                st.write("#### Detailierte Ergebnisse (Platzhalter)")
                for i, res in enumerate(st.session_state.results, start=1):
                    st.write(f"**{i}. Ergebnis:**")
                    st.json({
                        "Ersparnis (Euro)": round(res[0], 2),
                        "Rohdaten": res[1]  # Hier könnten z. B. Zeitreihen oder Details stehen
                    })